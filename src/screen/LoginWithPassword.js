...
const LoginWithPassword = ({navigation, route}) => {
  ...
  const handleSubmit = async () => {
    const device_id = await messaging().getToken();
    switch (tabPosition) {
      case 1:
        try {
          // preparing params
      ...
      case 2:
        try {
          ...
          // preparing params
          const params = {email: email, password: password, device_id};
  ...
  const handleForget = () => {
    navigation.navigate('ForgetPassword', {mobNum: mobile});
  };
